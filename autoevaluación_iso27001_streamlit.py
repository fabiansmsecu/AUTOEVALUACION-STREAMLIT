...
def generar_conclusiones_recomendaciones(texto_informe, nombre_evaluador):
    try:
        # Crear la conversación utilizando la API actualizada
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",  # O usa "gpt-4" si tienes acceso
            messages=[
                {"role": "system", "content": "Eres un experto en auditorías ISO 27001. Tu tarea es analizar el siguiente informe de autoevaluación, identificar las áreas de mejora y generar recomendaciones prácticas y detalladas para mejorar el cumplimiento de la norma ISO 27001."},
                {"role": "user", "content": f"Aquí está la autoevaluación realizada por el usuario:\n\n{texto_informe}\n\nGenera una conclusión que resuma los hallazgos y ofrece recomendaciones de alto valor que la empresa debería implementar para mejorar su cumplimiento con la norma ISO 27001."}
            ],
            max_tokens=3000  # Aumenta el límite de tokens para permitir respuestas más largas
        )
        # Extraer el contenido del mensaje de la respuesta
        conclusion = response.choices[0].message['content'].strip()

        # Añadir el párrafo promocional
        conclusion += f"\n\n**Nota sobre Deltech Audit:**\nDeltech Audit es una Firma de Auditoría y Consultoría con un alto énfasis en tecnología y más de 20 años de experiencia. Podemos ayudarle a validar esta autoevaluación o mejorarla con nuestra experiencia."

        return conclusion

    except Exception as e:
        messagebox.showerror("Error", f"No se pudo generar conclusiones y recomendaciones: {e}")
        return ""
...
def generar_informe_word(calificaciones, promedios_ponderados, calificacion_final, nombre_compania, nombre_evaluador,
    ...
    document.add_paragraph(f'Atentamente,\n{nombre_evaluador}', style='Heading 3')
    ...
    for aspecto, preguntas in calificaciones.items():
        ...
        for pregunta, calificacion in preguntas:
            ...
            texto_informe += f"{pregunta}: {rubricas[aspecto][pregunta][calificacion]}\n\n"
    ...
    # Conclusión general
    conclusion = generar_conclusiones_recomendaciones(texto_informe, nombre_evaluador)
